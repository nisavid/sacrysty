...
| `python3 conformance/check-fido-custody.py` | Synthetic one-shot custody workers | Success, failure, sibling-thread cancellation, and exact final-exec environment paths; the environment observer is a temporary value-free native fixture, not authenticator, plugin, native-library, or production qualification. |
