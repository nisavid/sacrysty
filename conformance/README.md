...
| `python3 conformance/check-fido-custody.py` | Synthetic one-shot custody workers | Success, failure, numeric resource bounds, main-thread invocation, sibling-thread cancellation, and exact final-exec environment paths; the environment observer is a temporary value-free native fixture, not authenticator, plugin, native-library, or production qualification. |
| `python3 conformance/check-source-inventory.py` | Producer commits and every inventoried source path | Pinned standalone source bundle, complete producer objects, tree membership, modes, blobs, byte lengths, and hashes; independent of candidate ancestry, with no signature, hosted availability, or review claim. |
| `python3 conformance/test-source-inventory.py` | Source-inventory identity and recoverability | Squashed shallow checkout without producer history, plus identity, path-set, and missing or damaged bundle failures. |
